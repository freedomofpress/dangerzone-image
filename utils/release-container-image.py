...
@click.option("--image-name", default=IMAGE_NAME)
@click.option(
...
def attest_release_sign(
    ...
    try:
        ...
        click.echo("📦 Installing cosign binary...")
        run(["uvx", "mazette", "install", "cosign"])
...
def attest_provenance(full_image, repository, workflow):
    ...
    result = subprocess.run(
        [
            sys.executable,
            str(PROJECT_ROOT / "utils" / "attest_provenance.py"),
            "--image",
            full_image,
            "--repository",
            repository,
