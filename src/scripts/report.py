...
def _oci_dag_dev(ref):
    """Turn an image ref into a URL on https://oci.dag.dev."""
    return f"[`{ref}`](https://oci.dag.dev/?image={ref})"
...
def _generate_build_info(date, root_ref, platform_info, platform_dirs):
    ...
    lines.append("\n")
    lines.append(f"- Root image: {_oci_dag_dev(root_ref)}`\n")
    ...
    for d in sorted(platform_dirs, key=lambda x: x.name):
        ...
        lines.append(f"- `{platform_label}` image: {_oci_dag_dev(platform_ref)}\n")
